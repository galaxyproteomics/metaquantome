...
def expand(args):
    ...
    df = io.read_and_join_files(mode, args.pep_colname, args.samp_grps,
    ...
    if mode == 'fn':
        results = functional_analysis(df=df, func_colname=args.func_colname, samp_grps=args.samp_grps, test=args.test,
                                      threshold=args.threshold, ontology=args.ontology,
                                      slim_down=args.slim_down, paired=args.paired,
                                      parametric=args.parametric, data_dir=args.data_dir, overwrite=args.overwrite,
                                      min_peptides=args.min_peptides, min_children_non_leaf=args.min_children_non_leaf)
    elif mode == 'tax':
        results = taxonomy_analysis(df=df, samp_grps=args.samp_grps, test=args.test, threshold=args.threshold,
                                    paired=args.paired,
                                    parametric=args.parametric, data_dir=args.data_dir, tax_colname=args.tax_colname,
                                    min_peptides=args.min_peptides, min_children_non_leaf=args.min_children_non_leaf)
    elif mode == 'taxfn':
    ...
    if args.outfile:
        cols = io.define_outfile_cols(args.samp_grps, args.ontology, mode, args.test)
