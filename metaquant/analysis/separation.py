...
#     # for two groups this is just the distance between centroids
#     avg_dist_bet_cent = avg_dist_between_group(centroids)
#
#     # within variance
#     mean_within = mean_sse_within_group(df, centroids, samp_grps)
#
#     return avg_dist_bet_cent / mean_within
#
#
...
#
#
# def mean_sse_within_group(df, centroids, samp_grps):
#     """
...
#         within[i] = np.square(df[samp_grps.sample_names[grp]].subtract(centroids[i], axis='index')).values.mean()
#     mean_within = np.sum(within)
#     return mean_within
#
